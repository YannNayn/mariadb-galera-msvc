...
extern "C" char wsrep_defaults_file[];
                                 ...
                                 bool                bypass)
{
  ...
  else
  {
    ...
    //char host[host_len];
    char    *host = (char  *)alloca(host_len *sizeof(char));
  ...
  //char user[user_len];
  char    *user = (char  *)alloca(user_len *sizeof(char));
...
static int sst_flush_tables(THD* thd)
{
  ...
  else
  {
    ...
    char    *real_name = (char  *)alloca(full_len *sizeof(char));
    sprintf(real_name, "%s/%s", mysql_real_data_home, base_name);
    //char tmp_name[full_len + 4];
    char    *tmp_name = (char  *)alloca((full_len + 4) *sizeof(char));
