...
#define my_delete_allow_opened my_delete
...
handlerton *__binlog_hton;
#ifdef WITH_WSREP
extern "C"
{
extern handlerton *_wsrep_hton();
handlerton * _binlog_hton()
{
	return __binlog_hton;
}
}
#define wsrep_hton _wsrep_hton()

#endif
#define binlog_hton _binlog_hton()
...
#ifdef _MSC_VER
#ifdef WITH_WSREP
  if (!wsrep_emulate_bin_log && (trans_has_updated_non_trans_table(thd) || (thd->variables.option_bits & OPTION_KEEP_LOG)))
#else
  if (trans_has_updated_non_trans_table(thd) || (thd->variables.option_bits & OPTION_KEEP_LOG))
#endif
