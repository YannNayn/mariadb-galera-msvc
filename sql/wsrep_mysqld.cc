...
my_bool __wsrep_drupal_282555_workaround = 1; // retry autoinc insert after dupkey
extern "C" my_bool *_wsrep_drupal_282555_workaround()
{
	return &__wsrep_drupal_282555_workaround;
}
#define wsrep_drupal_282555_workaround *_wsrep_drupal_282555_workaround()

my_bool __wsrep_certify_nonPK            = 1; // certify, even when no primary key
extern "C" my_bool *_wsrep_certify_nonPK()
{
	return &__wsrep_certify_nonPK;
}
...
//	my_bool __wsrep_debug                    = 0; // enable debug level logging
}
my_bool __wsrep_log_conflicts            = 0;

extern "C" my_bool *pwsrep_log_conflicts()
{
	return &__wsrep_log_conflicts;
}
...
my_bool __wsrep_load_data_splitting      = 1;
extern "C" my_bool *_wsrep_load_data_splitting()
{
	return &__wsrep_load_data_splitting;
}
...
long __wsrep_protocol_version = 2; // retry autoinc insert after dupkey
extern "C" long *_wsrep_protocol_version()
{
	return &__wsrep_protocol_version;
}
...
extern wsrep_status_t wsrep_apply_cb(void *const ctx,
                                     const void*const  buf, size_t const  buf_len,
                                     wsrep_seqno_t const global_seqno);

extern wsrep_status_t wsrep_commit_cb  (void  * const  ctx,
...
int wsrep_init()
{
  ...
  if ((rcode= wsrep_load(wsrep_provider, &__wsrep, wsrep_log_cb)) != WSREP_OK)
  {
...
extern "C" my_bool wsrep_new_cluster;
